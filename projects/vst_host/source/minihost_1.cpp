...
int main (int argc, char* argv[])
{
	...
	const char* effectfile = "test16.dll";
	...
	const char * drysoundfile ;
	const char * wetsoundfile ;
	int effect_type = 0;
	int numParams = 0;
	float * step;				//kroky posuvu jednotlivych parametrov, v percentach <0..1>
	...
	printf ("HOST> Open dry sound\n");
	WaveReader dry(drysoundfile);

	printf ("HOST> Open wet sound\n");
	WaveReader wet(wetsoundfile);

	printf ("HOST> Effect init sequence...\n");
	...
	printf ("HOST> Effect init sequence done\n");
...
*/	
	...
	step = new float[effect->numParams]
	
	switch (effect_type) {
		case 1	:	process_delay(); break;
		default	:	printf("HOST> Unsupported effect\n"); break;
	}
