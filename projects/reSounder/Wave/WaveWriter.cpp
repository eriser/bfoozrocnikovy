...
WaveWriter::WaveWriter(String ^ name, array<Byte> ^ header, long DataLength) {
	
	bits_per_sample = 0;

	//otvorime subor
	output = gcnew BinaryWriter(File::Open(name, FileMode::Create));
