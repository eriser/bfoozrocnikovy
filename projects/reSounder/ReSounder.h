...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->panel_Name->Size = System::Drawing::Size(80, 237);
			...
			this->panel_Step->Location = System::Drawing::Point(117, 4);
			...
			this->panel_Vol->Location = System::Drawing::Point(91, 4);
...
private: System::Void b_Effect_Click(System::Object^  sender, System::EventArgs^  e) {
			 if (openFileDialog3->ShowDialog() == System::Windows::Forms::DialogResult::OK) {
				//nacitame efekt
				openEffectFile(openFileDialog3->FileName);
			 }
		 }

private: bool openEffectFile(String^ name) {
			bool isEffectLoaded = true;
			try {
				host->openEffectFile(name);
			}
			catch (Exception ^ e) {
...
private: void paramsToProcess_reset() {
			 for (int i=0; i<maxParams; i++) {
				 for (int j=0; j<3; j++) {
					table_Params->Controls[j]->Controls[i]->Visible = false;	//skryjeme vsetko
				 }

				 ((System::Windows::Forms::CheckBox^)panel_Name->Controls[i])->Checked = true;	//zaskrtnutie vyberu
				 ((System::Windows::Forms::CheckBox^)panel_Name->Controls[i])->Text = "";		//text
				 ((System::Windows::Forms::CheckBox^)panel_Vol->Controls[i])->Checked = false;	//odskrtutie volume					
				 panel_Step->Controls[i]->Text = "10";		//krok
				 panel_Name->Controls[i]->Enabled = true;	//povolenie
				 
			 }

			 check_Fast->Visible = false;
			 check_Fast->Checked = true;
