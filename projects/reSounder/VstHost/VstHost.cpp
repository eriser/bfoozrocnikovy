...
void VstHost::processDelay(vector<float>* bestValues){
	...
	float	stepSize = (float)0.05;			//krok v anstavovani parametrov
...
//--------  vypocet  ------------------------------------------------------------------------
	int paramPom = 0;
	loader->setParam(0,(float)0.1);		//delay
	...
	for (int interval=0; interval<intervalNum; interval++) {	//cez vsetky intervaly
		...
		calculateRms(intervalRmsPom, intervalLength, intervalRmsWet[interval]);	//spocitame RMS pre aktualy interval
	}
